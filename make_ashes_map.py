#!/usr/bin/env python3
"""
make_ashes_map.py - Generate ultra-high-resolution ASHES vent field map

Creates a detailed bathymetric map of the ASHES vent field using 1cm LASS lidar data.

Usage:
    uv run python make_ashes_map.py
"""
...
def plot_ashes_map(x, y, z, output_path: Path, subsample: int = 1):
    """Create shaded relief map of ASHES vent field with vent annotations."""
    ...
    gl.xlocator = mticker.FixedLocator(
        np.arange(-130.016, -130.011, 0.0005)
    )
    gl.ylocator = mticker.FixedLocator(
        np.arange(45.931, 45.936, 0.0005)
    )
    ...
    caption = (
        "ASHES vent field at Axial Seamount, mapped with 1 cm resolution LASS lidar "
        "bathymetry (MBARI, 2025). Shaded relief with 1-meter depth contours. Markers "
        "indicate vent locations colored by temperature classification from the "
    ...
    output_file = output_path / f"ashes_detail_map{resolution_note}.png"
...
def main():
    print("=" * 60)
    print("ASHES Vent Field Detail Map")
    print("=" * 60)
    ...
    output_file = plot_ashes_map(x, y, z, OUTPUT_DIR, subsample=subsample)
